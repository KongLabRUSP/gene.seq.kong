...
##3, combine reads into regions. THis step gets done in seconds.
$ python ../../DMRfinder/combine_CpG_sites.py -o combined.csv S1.cov S2.cov S3.cov S4.cov S5.cov S6.cov

#4, run DMRfinder.r  ####pseduo group set. result.csv should be discarded.
...
#optional, install DSS (only for first time run) Use this link instructions to fix some problems.
...
##run ChIPseeker to add gene annotation
R
source("https://bioconductor.org/biocLite.R")
