...
downreg50 <- dt1[(nrow(dt1) - 49):nrow(dt1)]
...
p1 <- ggplot(data = dt2) +
  ...
  scale_y_discrete("Gene  and Region",
                   expand = c(0, 0)) +
  ggtitle("Log2(TRAMP/C57) Methylation of Top 50 Upregulated \n and Top 50 Downregulated Regions")  +
  theme(axis.text.x = element_text(angle = 20),
        plot.title = element_text(hjust = 0.5),
        legend.position = "top")

tiff(filename = "tmp/MeDIP_Wenji_Top50Up_Top50Down.tiff",
     height = 7,
     width = 7,
     units = 'in',
     res = 300,
